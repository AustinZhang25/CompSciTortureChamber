...
public class Deer extends Animal{
    ...
    public Deer(Location l, World w) {
        super(l,w);
        myLifeSpan = 6;
    ...
    public void reproduce() {
        ...
        if (alive && deerAdjacent && hasEaten){
                ...
                myWorld.getCreatureList().add(new Deer(newLocation, myWorld));
            }
        }
    ...
    public void Eat(){
        ...
        for (int i = 0; i<arr.size(); i++){
            ...
            if(arr.get(i) instanceof Grass && Math.abs(deerX - otherX) <= 1 && Math.abs(deerY - otherY) <= 1) {
...
//        if (Deer.super.getMyLocation().getX()+1)
    ...
    public void Move(){
        int newX = deerX+rgen.nextInt(-2,2);
        int newY = deerX+rgen.nextInt(-2,2);
        if (!isFree(new Location(newX, newY))){
            newX = deerX+rgen.nextInt(-2,2);
            newY = deerX+rgen.nextInt(-2,2);
        }
        Location newLocation = new Location(newX, newY);
